...
>
  ...
  {#if sua}
    <p class="mb-1 text-[0.7rem] font-bold uppercase tracking-wide text-carvao/60">Du sagst</p>
    <button
      type="button"
      onclick={tocarSuaFala}
      aria-label={revelado ? `Abspielen: ${sua.es}` : 'Satz aufdecken'}
    ...
    >
      ...
      {:else}
        ...
        <span class="mt-1 block text-[0.8rem] font-semibold text-oceano"
          >Das kannst du schon — zum Aufdecken tippen</span
        >
      {/if}
    </button>
  {/if}
  ...
  {#if falhou}
    <p role="status" class="mt-2 text-[0.8rem] font-semibold text-terracota">
      ⚠️ Für den hier gibt's kein Audio — lad den Spickzettel vor dem Abflug runter.
    </p>
  {/if}
</article>
